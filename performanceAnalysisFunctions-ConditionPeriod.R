...
Output_lme_performance_nurses_study <- function(PVT, OB, GNG, post_hoc){
  ...
  hitrate_model <- lme(HitRate ~ Shift * Condition_period *  TimeBin,random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                       data=GNG2)
  ...
  fp_rate_model <- lme(FalsePositive ~ Shift * Condition_period *  TimeBin,random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                       data=GNG2)
  
  sigList_output2 <- pval_postHoc_OutPut("GNG false positve", fp_rate_model, GNG2, post_hoc)
  
  
  gng_rt_model <- lme(avgResponseTimeCorrect ~ Shift * Condition_period *  TimeBin,random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                       data=GNG2)
  
  sigList_output3 <- pval_postHoc_OutPut("GNG mean response time", gng_rt_model, GNG2, post_hoc)
  
  gng_rt_model2 <- lme(medResponseTimeCorrect ~ Shift * Condition_period *  TimeBin,random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                      data=GNG2)
  ...
  correct_accuracy_OBnormal <- lme(PercentCorrect ~ Shift * Condition_period *  TimeBin, random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                                data=OB_normal)
  sigList_output5 <- pval_postHoc_OutPut("OB accuracy", correct_accuracy_OBnormal, OB_normal, post_hoc)
  
  
  correct_match_OBnormal <- lme(CorrectMatch ~ Shift * Condition_period *  TimeBin, random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                       data=OB_normal)
  sigList_output6 <- pval_postHoc_OutPut("OB correct matches", correct_match_OBnormal, OB_normal, post_hoc)
  
  
  correct_NoMatch_OBnormal <- lme(CorrectNoMatch ~ Shift * Condition_period *  TimeBin,random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                                data=OB_normal)
  sigList_output7 <- pval_postHoc_OutPut("OB no correct matches", correct_NoMatch_OBnormal, OB_normal, post_hoc)
  
  
  OB_rt_model <- lme(avgResponseTimeCorrect ~ Shift * Condition_period *  TimeBin,random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
  ...
  OB_rt_model2 <- lme(medResponseTimeCorrect~ Shift * Condition_period *  TimeBin,random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
  ...
  if(FALSE){
    correct_accuracy_OBnormal <- lme(nCorrect ~ Shift * Condition_period *  TimeBin, random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                                     data=OB2)
    sigList_output5 <- pval_postHoc_OutPut("OB accuracy", correct_accuracy_OBnormal, OB_normal, post_hoc)
    ...
    #OB_rt_model2 <- lme(avgResponseTime~ Shift * Condition_period *  TimeBin,random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, ,
  ...
  PVT2 <- subset(PVT, !is.na(TimeBin) & !is.na(avgResponseTime) & HasMaxTrials == "TRUE" & Condition_period != "NA_NA")
  
  correct_PVT <- lme(accuracy ~ Shift * Condition_period *  TimeBin, random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                                data=PVT2)
  ...
  PVT_rt_model <- lme(avgResponseTime ~ Shift * Condition_period *  TimeBin, random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                     data=PVT2)
  
  sigList_output14 <- pval_postHoc_OutPut("PVT mean response time", PVT_rt_model, PVT2, post_hoc)
  
  
  PVT_rt_model2 <- lme(medResponseTime ~ Shift * Condition_period *  TimeBin, random = ~1|SubjectID/Condition_period/TimeBin, control=ctrl, 
                      data=PVT2)
